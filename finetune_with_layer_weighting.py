...
from nnm_recovery_weight import compute_recovery_layer_weights
...
def prepare_nnm(args, tokenizer, raw_student, teacher_model, dataset, device):
    ...
    lw_tensor = torch.zeros(len(s_mid), device=device, dtype=torch.float32)
    ...
    if rank == 0:
        ...
        # ── recovery-slope layer weights (re-iterate the loader, cheap) ──
        ec_loader = DataLoader(dataset["train"], shuffle=True, drop_last=True,
                               batch_size=args.batch_size,
                               num_workers=args.num_workers,
                               collate_fn=dataset["train"].collate)
        rank0_lw = compute_recovery_layer_weights(
            teacher=teacher_model,
            dataloader=_flatten_batches(ec_loader),
            s_mid=s_mid,
            t_mid=t_mid,
            n_s_layers=n_s_layers,
            n_t_layers=n_t_layers,
            max_batches=min(50, args.nnm_centroid_batches),
            device=teacher_device,
            debug_print=print_rank,
        )
        for i, s_lid in enumerate(s_mid):
            lw_tensor[i] = rank0_lw[s_lid]

    if world > 1:
        for s_lid in s_mid:
            dist.broadcast(t_centroids[s_lid], src=0)
        dist.broadcast(R, src=0)
        dist.broadcast(lw_tensor, src=0)
        dist.barrier()
        print_rank(f"[NNM] broadcast complete across {world} ranks")

    layer_weights = {s_lid: float(lw_tensor[i].item())
                     for i, s_lid in enumerate(s_mid)}
    print_rank(f"[NNM] recovery layer weights: {layer_weights}")
